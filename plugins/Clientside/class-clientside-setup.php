...
class Clientside_Setup {
	...
	static function action_enqueue_admin_styles() {
		...
		// Always-use CSS
		wp_enqueue_style( 'clientside-admin-css', plugins_url( 'css/clientside-admin-1.9.6.css', __FILE__ ), array(), null );
		...
		if ( Clientside_Options::get_saved_option( 'enable-admin-theme' ) ) {
			...
			// Theme CSS
			wp_enqueue_style( 'clientside-theme-css', plugins_url( 'css/clientside-admin-theme-1.9.6.min.css', __FILE__ ), array( 'clientside-admin-css', 'thickbox' ), null );

			// Additional external plugin support, when applicable
			if ( Clientside_Options::get_saved_option( 'enable-plugin-support' ) ) {

				// Global
				wp_enqueue_style( 'clientside-plugin-support-css', plugins_url( 'css/clientside-plugin-support-1.9.6.min.css', __FILE__ ), array( 'clientside-theme-css' ), null );

				// Advanced Custom Fields (free + pro)
				if ( class_exists( 'acf' ) ) {
					wp_enqueue_style( 'clientside-plugin-support-css-acf', plugins_url( 'css/plugin-support/clientside-acf-1.9.6.min.css', __FILE__ ), array( 'clientside-theme-css' ), null );
				}

				// WooCommerce
				if ( class_exists( 'WooCommerce' ) ) {
					wp_enqueue_style( 'clientside-plugin-support-css-woocommerce', plugins_url( 'css/plugin-support/clientside-woocommerce-1.9.6.min.css', __FILE__ ), array( 'clientside-theme-css' ), null );
				}

				// Layers WP
				if ( defined( 'LAYERS_VERSION' ) ) {
					wp_enqueue_style( 'clientside-plugin-support-css-layerswp', plugins_url( 'css/plugin-support/clientside-layerswp-1.9.6.min.css', __FILE__ ), array( 'clientside-theme-css' ), null );
				}

				// Jetpack
				if ( defined( 'JETPACK__VERSION' ) ) {
					wp_enqueue_style( 'clientside-plugin-support-css-jetpack', plugins_url( 'css/plugin-support/clientside-jetpack-1.9.6.min.css', __FILE__ ), array( 'clientside-theme-css' ), null );
				}

				// Google Analytics Dashboard
				if ( defined( 'GADWP_CURRENT_VERSION' ) ) {
					wp_enqueue_style( 'clientside-plugin-support-css-gadash', plugins_url( 'css/plugin-support/clientside-gadash-1.9.6.min.css', __FILE__ ), array( 'clientside-theme-css' ), null );
				}

				// HTML Editor Syntax Highlighter
				if ( defined( 'HESH_LIBS' ) ) {
					wp_enqueue_style( 'clientside-plugin-support-css-htmleditorsyntax', plugins_url( 'css/plugin-support/clientside-htmleditorsyntax-1.9.6.min.css', __FILE__ ), array( 'clientside-theme-css' ), null );
				}

				// WP Statistics
				if ( defined( 'WP_STATISTICS_VERSION' ) ) {
					wp_enqueue_style( 'clientside-plugin-support-css-wpstatistics', plugins_url( 'css/plugin-support/clientside-wpstatistics-1.9.6.min.css', __FILE__ ), array( 'clientside-theme-css' ), null );
				}

				// Revolution Slider
				if ( defined( 'REVSLIDER_TEXTDOMAIN' ) ) {
					wp_enqueue_style( 'clientside-plugin-support-css-revslider', plugins_url( 'css/plugin-support/clientside-revslider-1.9.6.min.css', __FILE__ ), array( 'clientside-theme-css' ), null );
				}

				// Divi (builder & theme)
				if ( defined( 'ET_BUILDER_PLUGIN_VERSION' ) || function_exists( 'et_get_theme_version' ) ) {
					wp_enqueue_style( 'clientside-plugin-support-css-divi', plugins_url( 'css/plugin-support/clientside-divi-1.9.6.min.css', __FILE__ ), array( 'clientside-theme-css' ), null );
				}

				// Super Forms
				if ( class_exists( 'SUPER_Forms' ) ) {
					wp_enqueue_style( 'clientside-plugin-support-css-superforms', plugins_url( 'css/plugin-support/clientside-superforms-1.9.6.min.css', __FILE__ ), array( 'clientside-theme-css' ), null );
				}
	...
	static function action_enqueue_admin_scripts() {
		...
		// Clientside theme JS
		wp_enqueue_script( 'clientside-admin-js', plugins_url( 'js/clientside-admin.js', __FILE__ ), $dependencies, '1.9.6' );
	...
	static function action_enqueue_login_styles() {
		...
		if ( Clientside_Options::get_saved_option( 'enable-login-theme' ) ) {
			wp_enqueue_style( 'clientside-login-css', plugins_url( 'css/clientside-login-1.9.6.min.css', __FILE__ ), array(), null );
		}
	...
	static function action_enqueue_login_scripts() {
		...
		// Only if login page theming is enabled
		if ( Clientside_Options::get_saved_option( 'enable-login-theme' ) ) {
			wp_enqueue_script( 'clientside-login-js', plugins_url( 'js/clientside-login.js', __FILE__ ), array( 'jquery' ), '1.9.6' );
		}
	...
	static function action_enqueue_editor_styles() {
		...
		// Load the stylesheet
		add_editor_style( plugins_url( 'css/clientside-editor-1.9.6.min.css', __FILE__ ) );
	...
	static function filter_add_body_classes( $body_classes ) {
		...
		if ( Clientside_Options::get_saved_option( 'logo-image' ) && ! Clientside_Options::get_saved_option( 'hide-menu-logo' ) ) {
			...
			$new_classes[] = 'clientside-no-custom-logo';
		}
