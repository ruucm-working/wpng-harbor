...
class Clientside_Options {
	...
	static function get_option_info( $option_slug = '' ) {
		...
		$options['theme-menu'] = array_merge(
			...
			array(
				'name' => 'theme-menu',
				'title' => _x( 'Admin Menu color scheme', 'Option title', 'clientside' ),
				'type' => 'radio',
				'options' => array(
					'muted' => __( 'Muted (default)', 'clientside' ),
					'color1' => __( 'Dark (beta)', 'clientside' )
				),
				'default' => 'muted',
		...
		$options['enable-metabox-close-button'] = array_merge(
			...
			array(
				...
				'title' => _x( 'Metabox close buttons', 'Option title', 'clientside' ),
				'secondary-title' => __( 'Enable', 'clientside' ),
				'type' => 'checkbox',
				'role-based' => true,
				'default' => 1,
		...
		$options['logo-image'] = array_merge(
			...
			array(
				...
				'help' => _x( 'Will appear on the login page and above the admin menu (optional). Leave empty to use the default WordPress logo (login page) and View Site button above the admin menu.', 'Option description', 'clientside' ),
				'type' => 'image',
				'default' => ''
			)
		);
	...
	static function get_title_with_hint_tooltip_html( $field ) {
		...
		if ( ! Clientside::is_enabled() || ! Clientside_Options::get_saved_option( 'enable-admin-theme' ) || ! isset( $field['help'] ) || ! $field['help'] ) {
			return $field['title'];
		}
	...
	static function display_form_field( $args = array() ) {
		...
		if ( $field['help'] && ( ! Clientside::is_enabled() || ! Clientside_Options::get_saved_option( 'enable-admin-theme' ) ) ) {
